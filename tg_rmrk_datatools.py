...
def to_ksm(price, fee_type="kanaria"):
    if fee_type == "kanaria":
        return Decimal(price) / Decimal(10**12) / Decimal(1 - kanaria_fee / 100)
    else:
        return Decimal(price) / Decimal(10**12) / Decimal(1 - singular_fee / 100)
...
def update_ksm_exchange_rate(bot, job):
    ...
    db = conn.cursor()
    try:
        ksm_exchange_rate = Decimal(cryptocompare.get_price("KSM", currency='USD')['KSM']['USD'])
        db.execute(f"UPDATE tg_ksm_exchange_rate SET ksm_exchange_rate={ksm_exchange_rate:.2f}")
        conn.commit()
    ...
    if kanaria_birds_ids_str not in nft_id:
        if 'properties' in nft_metadata:
            ...
            for p in nft_metadata['properties']:
                ...
                if 'percentage' in p:
                    try:
                        nft_metadata['properties'][p]['value'] = str(round(float(nft_metadata['properties'][p]['value']), 2)) + '%'
                    except:
                        pass
                send_text += f"{p}: <b>{nft_metadata['properties'][p].get('value','')}</b>\n"
            send_text = send_text.replace("legendary","🔸legendary").replace("epic","🔹epic").replace("rare","🔺rare").replace("uncommon","▫️uncommon")
            send_text += '\n'
...
def fetch_metadata(db, nft_id, metadata_url):
        nft_metadata = {}
        db.execute(f"SELECT metadata FROM tg_nft_metadata WHERE nft_id='{nft_id}';")
        if db.rowcount != 0:
            nft_metadata = db.fetchone()[0]
        elif metadata_url:
            try:
                nft_metadata = requests.get(metadata_url.replace('ipfs://',ipfs_gateway_url)).json()
            except:
                nft_metadata = {}
            if kanaria_birds_ids_str in nft_id:
                nft_metadata['image'] = requests.get(f'{kanaria_nft_api_url}{nft_id}').json()['image'] + f"?random={random.randint(0,42)}"
                if 'theme' not in nft_metadata and 'Theme' not in nft_metadata:
                    nft_metadata['theme'] = 'Defaultyellow'
            nft_metadata['image'] = nft_metadata.get('image','').replace('ipfs://',ipfs_gateway_url)
            json_metadata = json.dumps(nft_metadata).replace("'", "''")
            db.execute(f"INSERT INTO tg_nft_metadata(nft_id, metadata) VALUES('{nft_id}','{json_metadata}');")
        return nft_metadata