...
def estimate_item(db, nft_rarity, nft_type, nft_name):
    ...
    db.execute(f"SELECT old::bigint FROM nft_changes_v2 WHERE optype = 'BUY' AND field='forsale' AND nft_id IN (SELECT nft_id FROM tg_items_info WHERE rarity = '{nft_rarity}' AND type = '{nft_type}');")
    if db.rowcount > 0:
        rarity_type_prices = [x[0] for x in db.fetchall()]
        rarity_type_prices_min, rarity_type_prices_max = to_ksm(min(rarity_type_prices)), to_ksm(max(rarity_type_prices))
        rarity_type_prices_avg = to_ksm(sum(rarity_type_prices)/len(rarity_type_prices))
    else:
        rarity_type_prices_min, rarity_type_prices_max, rarity_type_prices_avg = 0, 0, 0
    db.execute(f"SELECT old::bigint FROM nft_changes_v2 WHERE optype = 'BUY' AND field='forsale' AND nft_id IN (SELECT nft_id FROM tg_items_info WHERE rarity = '{nft_rarity}' AND type = '{nft_type}' AND name = '{nft_name}');")
    if db.rowcount > 0:
        rarity_type_name_prices = [x[0] for x in db.fetchall()]
        rarity_type_name_prices_min, rarity_type_name_prices_max = to_ksm(min(rarity_type_name_prices)), to_ksm(max(rarity_type_name_prices))
        rarity_type_name_prices_avg = to_ksm(sum(rarity_type_name_prices)/len(rarity_type_name_prices))
    else:
...
def bird_items_on_chain_block(db, nft_id, items_type, block=-1):
    ...
    db.execute(f"SELECT id FROM nft_children_v2 WHERE nft_id='{nft_id}' AND id IN (SELECT id FROM nfts_v2 WHERE collection IN ('{items_type_ids_str}') AND burned!='true');")
    current_items = [x[0] for x in db.fetchall()]
    if block != -1:
        db.execute(f"SELECT nft_id, old, new FROM nft_changes_v2 WHERE optype='SEND' AND field='owner' AND (old='{nft_id}' OR new='{nft_id}') AND block >= {block} AND nft_id IN (SELECT id FROM nfts_v2 WHERE collection IN ('{items_type_ids_str}') AND burned!='true') ORDER BY block DESC;")
...
def estimate_bird(db, nft_id, estimation_type="full"):
    ...
    if db.rowcount > 0:
        ...
        db.execute(f"SELECT * FROM tg_birds_traits_amount WHERE nft_id = '{nft_id}';")
        bird_trait_info = db.fetchone()
        db.execute(f"SELECT * FROM tg_birds_common_rate WHERE nft_id = '{nft_id}';")
        ...
        for i in range(2, len(bird_info[:-3])):
            if re.sub(r"[^0-9a-f-]", "", bird_info[i]) == bird_info[i]:
                bird_info[i] = ''.join([chr(int(x,16)) for x in bird_info[i].split('-')])
        ...
        send_text = send_text.replace('var0','iridescent').replace('var1','plain').replace('var2','pinstrip').replace('var3','speckled').replace('var4','solid')
        ...
        if estimation_type != "header":
            ...
            db.execute(f"SELECT nft_id, block, old::bigint FROM nft_changes_v2 WHERE optype='BUY' AND field='forsale' AND nft_id IN (SELECT nft_id FROM tg_birds_common_rate WHERE trait_score BETWEEN {bird_total_info[1]*0.95} AND {bird_total_info[1]*1.05}) ORDER BY block DESC LIMIT 10;")
            if db.rowcount == 0:
                # Similar birds = Birds with score +/- 10%
                db.execute(f"SELECT nft_id, block, old::bigint FROM nft_changes_v2 WHERE optype='BUY' AND field='forsale' AND nft_id IN (SELECT nft_id FROM tg_birds_common_rate WHERE trait_score BETWEEN {bird_total_info[1]*0.9} AND {bird_total_info[1]*1.1}) ORDER BY block DESC LIMIT 10;")
                if db.rowcount == 0:
                    not_enough = True
            ...
            if not not_enough:
                ...
                for similar_bird_sold in similar_birds_sold:
                    similar_bird_items = bird_items_on_chain_block(db, similar_bird_sold[0], 'items', similar_bird_sold[1])
                    similar_total_items_prices = []
                    for item_id in similar_bird_items:
                        db.execute(f"SELECT rarity, type, name FROM tg_items_info WHERE nft_id = '{item_id}';")
                        item_rarity, item_type, item_name = db.fetchone()
                        prices = estimate_item(db, item_rarity, item_type, item_name)[1:4]
                        similar_total_items_prices.append(prices)
                    similar_bird_max, similar_bird_min, similar_bird_avg = to_ksm(similar_bird_sold[2]) - sum([x[0] for x in similar_total_items_prices]), to_ksm(similar_bird_sold[2]) - sum([x[1] for x in similar_total_items_prices]), to_ksm(similar_bird_sold[2]) - sum([x[2] for x in similar_total_items_prices])
                    if similar_bird_min > 1:
                        similar_birds_prices.append([similar_bird_min, similar_bird_avg, similar_bird_max])
                if similar_birds_prices:
                    total_birds_min, total_birds_avg, total_birds_max = min([x[0] for x in similar_birds_prices]), sum([x[1] for x in similar_birds_prices]) / len(similar_birds_prices), max([x[2] for x in similar_birds_prices])
            ...
            total_items_min, total_items_max, total_items_avg = sum([x[0] for x in total_items_prices]), sum([x[1] for x in total_items_prices]), sum([x[2] for x in total_items_prices])
    ...
    return send_text, total_birds_min + total_items_min, total_birds_max + total_items_max, total_birds_avg + total_items_avg