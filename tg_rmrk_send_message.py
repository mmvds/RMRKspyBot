...
def send_message(conn, bot, db, nft_metadata, send_text, messages_list):
    ...
    while send_again and send_attempt < 10:
        ...
        if nft_metadata.get('image',''):
            ...
            db.execute(f"SELECT * FROM tg_images WHERE image_id='{nft_metadata['image'].split('/')[-1]}';")
            if db.rowcount > 0:
                if db.fetchone()[1]:
            ...
            if not try_text:
                try:
                    bot.send_photo(chat_id=messages_list[0], photo=photo_object, caption=send_text, parse_mode=ParseMode.HTML, timeout=30)
                except Exception as e: 
                    print(time.ctime(), round(time.time()), str(messages_list), ':send_mail:',e)
                    print(nft_metadata['image'])
                    if len(messages_list[1]) > 20:
                        nft_id = messages_list[1]
                    else:
                        nft_id = messages_list[2]
                    if ('file identifier' in str(e)) or ('Failed to get http url content' in str(e)):
                        if kanaria_birds_ids_str in nft_id:
                            db.execute(f"DELETE FROM tg_nft_metadata WHERE nft_id='{nft_id}';")
                            conn.commit()
                        ...
                        db.execute(f'UPDATE tg_users set is_active=False where id={messages_list[0]};')
                    else:
                        try_text = True
        else:
            try_text = True
    ...
    if try_text or send_attempt >= 10:
        ...
        try:
            bot.send_message(chat_id=messages_list[0], text=send_text, parse_mode=ParseMode.HTML)
        except Exception as e: 
            print(time.ctime(), round(time.time()), str(messages_list), ':send_mail:',e)
...
def send_record_message_to_channel(conn, bot, db, record_type, record_period, birds_record):
    nft_price, nft_id, nft_block = birds_record
    db.execute("SELECT * FROM tg_ksm_exchange_rate")
    ksm_exchange_rate = db.fetchone()[0]

    if record_type == 'item':
        db.execute(f"SELECT rarity, type, name FROM tg_items_info WHERE nft_id = '{nft_id}';")
        nft_rarity, nft_type, nft_name = db.fetchone()
        nft_name = nft_name.replace("'","''")
        db.execute(f"SELECT metadata FROM nfts_v2 WHERE id = '{nft_id}';")
        send_text, nft_metadata = extract_header_info(db, nft_id, db.fetchone()[0])
    ...
    send_message(conn, bot, db, nft_metadata, send_text, [tg_allowed_channels[0], nft_id])
    conn.commit()
...
def send_messages(bot, job):
    ...
    db.execute(f"DELETE FROM tg_changes_messages WHERE user_id NOT IN (SELECT id FROM tg_users WHERE is_active);")
    conn.commit()
    ...
    for send_change_messages in send_changes_messages:
        ...
        db.execute(f"DELETE FROM tg_changes_messages WHERE user_id={user_id} AND nft_id='{nft_id}' AND block={block} AND field='{field}';")
        ...
        if optype == 'LIST' and field == 'forsale':
            ...
            if version == 'v1':
                ...
                if db.rowcount != 0:
                    ...
                    db.execute(f"SELECT * FROM nfts_v2 WHERE id = '{new_value}';")
                    if db.rowcount != 0:
        ...
        send_message(conn, bot, db, nft_metadata, send_text, send_change_messages)
        conn.commit()

    #Send listed NFTs messages
    db.execute(f"DELETE FROM tg_forsale_messages WHERE user_id NOT IN (SELECT id FROM tg_users WHERE is_active);")
    conn.commit()
    ...
    for send_forsale_message in send_forsale_messages:
        ...
        db.execute(f"DELETE FROM tg_forsale_messages WHERE user_id={send_forsale_message[0]} AND type='{send_forsale_message[1]}' AND nft_id='{nft_id}' AND block={send_forsale_message[5]};")
        send_text, nft_metadata = extract_header_info(db, nft_id, send_forsale_message[6])
        usd_price = Decimal(send_forsale_message[4]) * Decimal(ksm_exchange_rate)
        ...
        send_message(conn, bot, db, nft_metadata, send_text, send_forsale_message)
        conn.commit()
    
    #Send sold NFTs messages
    db.execute(f"DELETE FROM tg_buy_messages WHERE user_id NOT IN (SELECT id FROM tg_users WHERE is_active);")
    conn.commit()
    ...
    for send_buy_message in send_buy_messages:
        db.execute(f"DELETE FROM tg_buy_messages WHERE user_id={send_buy_message[0]} AND type='{send_buy_message[1]}' AND nft_id='{send_buy_message[2]}' AND block={send_buy_message[4]};")
        nft_id = send_buy_message[2]
        send_text, nft_metadata = extract_header_info(db, nft_id, send_buy_message[5])
