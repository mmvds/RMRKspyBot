...
SELECT DISTINCT(new) FROM nft_changes_v2 WHERE optype = 'SEND' AND field = 'owner' AND block > {tg_lastblock_v2} AND new IN (SELECT id FROM nfts_v2 WHERE collection IN ('{kanaria_birds_ids_str}'));'''
    ...
    db.execute(
        f"DELETE FROM tg_nft_metadata WHERE nft_id in ('{changed_birds_ids_str}');")
    conn.commit()

    for changed_bird in changed_birds_ids:
        db.execute(
            f"SELECT metadata FROM nfts_v2 WHERE id = '{changed_bird}';")
        nft_metadata = fetch_metadata(db, changed_bird, db.fetchone()[0])

    db.execute(
        f"SELECT DISTINCT(owner) FROM nfts_v2 WHERE burned != '' AND collection IN ('{kanaria_gems_ids_str}') AND updatedatblock > {tg_lastblock_v2} AND owner IN (SELECT id FROM nfts_v2 WHERE collection IN ('{kanaria_birds_ids_str}'));")
    ...
    db.execute(
        f"DELETE FROM tg_birds_gems_info WHERE bird_id in ('{birds_with_burned_gems_ids_str}');")
    conn.commit()
...
def update_birds_gems_items(conn, db, tg_lastblock_v1, tg_lastblock_v2):
    ...
    for new_bird in new_birds:
        ...
        if 'properties' in nft_metadata:
            ...
            nft_theme = nft_metadata['properties'].get('theme', {'value': 'Defaultyellow'})[
                'value'].strip().lower()
        ...
        db.execute(
            f"SELECT parts FROM nft_resources_v2 WHERE id IN ('{bird_resources_str}');")
        ...
        for bird_resource_parts in bird_resources_parts:
            for bird_resource_part in bird_resource_parts:
                if "top_rare" in bird_resource_part or "tail_rare" in bird_resource_part:
                    bird_parts_dict['tail'] = bird_resource_part.split("_")[
                        0].lower()
    ...
    if db.rowcount > 0:
        ...
        for bird_traits_info in birds_traits_info:
            ...
            total_birds += 1
            total_birds_dict['rarity'][nft_rarity] = total_birds_dict['rarity'].get(
                nft_rarity, 0) + 1
            total_birds_dict['theme'][nft_theme] = total_birds_dict['theme'].get(
                nft_theme, 0) + 1
            total_birds_dict['head'][nft_head] = total_birds_dict['head'].get(
                nft_head, 0) + 1
            total_birds_dict['eyes'][nft_eyes] = total_birds_dict['eyes'].get(
                nft_eyes, 0) + 1
            total_birds_dict['body'][nft_body] = total_birds_dict['body'].get(
                nft_body, 0) + 1
            total_birds_dict['tail'][nft_tail] = total_birds_dict['tail'].get(
                nft_tail, 0) + 1
            total_birds_dict['wingleft'][nft_wingleft] = total_birds_dict['wingleft'].get(
                nft_wingleft, 0) + 1
            total_birds_dict['wingright'][nft_wingright] = total_birds_dict['wingright'].get(
                nft_wingright, 0) + 1
            total_birds_dict['feet'][nft_feet] = total_birds_dict['feet'].get(
                nft_feet, 0) + 1
            total_birds_dict['resource_amount'][nft_resource_amount] = total_birds_dict['resource_amount'].get(
                nft_resource_amount, 0) + 1

        # Calculate trait scores
        for bird_traits_info in birds_traits_info:
            nft_id, nft_rarity, nft_theme, nft_name, nft_head, nft_eyes, nft_body, nft_tail, nft_wingleft, nft_wingright, nft_feet, nft_resource_amount, nft_trait_score, nft_trait_place = bird_traits_info
            birds_score_dict[nft_id] = 2 * total_birds / \
                total_birds_dict['rarity'][nft_rarity]
            birds_score_dict[nft_id] += 1 * total_birds / \
                total_birds_dict['theme'][nft_theme]
            birds_score_dict[nft_id] += 1 * total_birds / \
                total_birds_dict['head'][nft_head]
            birds_score_dict[nft_id] += 1 * total_birds / \
                total_birds_dict['eyes'][nft_eyes]
            birds_score_dict[nft_id] += 3 * total_birds / \
                total_birds_dict['body'][nft_body]
            birds_score_dict[nft_id] += 1 * total_birds / \
                total_birds_dict['tail'][nft_tail]
            birds_score_dict[nft_id] += 1 * total_birds / \
                total_birds_dict['wingleft'][nft_wingleft]
            birds_score_dict[nft_id] += 1 * total_birds / \
                total_birds_dict['wingright'][nft_wingright]
            birds_score_dict[nft_id] += 1 * total_birds / \
                total_birds_dict['feet'][nft_feet]
            birds_score_dict[nft_id] += 3 * total_birds / \
                total_birds_dict['resource_amount'][nft_resource_amount]
        ...
        top_place = 0
        for bird_id in sorted(
                birds_score_dict, key=birds_score_dict.get, reverse=True):
            top_place += 1
            db.execute(
                f"UPDATE tg_birds_info set trait_score = {birds_score_dict[bird_id]:.2f}, trait_place={top_place} WHERE nft_id='{bird_id}';")
        conn.commit()
    ...
    if db.rowcount > 0:
        ...
        for bird_gems_info in birds_gems_info:
            for gem_id in bird_gems_info[1]:
                total_gems += 1
                db.execute(
                    f"SELECT collection FROM nfts_v2 WHERE id = '{gem_id}';")
                gem_collection = db.fetchone()[0]
                total_gems_dict[gem_collection] = total_gems_dict.get(
                    gem_collection, 0) + 1
        for bird_gems_info in birds_gems_info:
        ...
        top_place = 0
        for bird_gems_id in sorted(
                gems_score_dict, key=gems_score_dict.get, reverse=True):
            top_place += 1
            db.execute(
                f"UPDATE tg_birds_gems_info set trait_score = {gems_score_dict[bird_gems_id]:.2f}, trait_place={top_place} WHERE bird_id='{bird_gems_id}';")
        conn.commit()
        ...
        for bird_common_rates in birds_common_rates:
            birds_common_rates_dict[bird_common_rates[0]
                                    ] = bird_common_rates[1]
        top_place = 0
    ...
    for new_item in new_items:
        ...
        if 'properties' in nft_metadata:
            ...
            nft_name = nft_metadata.get('name', '').replace("'", "''")
            db.execute(
                f"INSERT INTO tg_items_info(nft_id, rarity, type, name) VALUES('{new_item[0]}', '{nft_rarity}','{nft_type}','{nft_name}');")
            conn.commit()
        else:
            print(nft_metadata)
            print(new_item[0])
    ...
    for period in records:
        ...
        if db.rowcount > 0:
            ...
            if records[period]['item'][0] != birds_record[1] or records[period]['item'][1] != birds_record[2]:
                db.execute(
                    f"UPDATE tg_record SET nft_id = '{birds_record[1]}', block = {birds_record[2]} WHERE record_period = '{period}' AND record_type = 'item';")
                to_send_records['item'][period] = birds_record

        db.execute(
            f"SELECT old::bigint, nft_id, block FROM nft_changes_vlite WHERE optype ='BUY' AND field='forsale' AND block >= (SELECT block FROM tg_block_history WHERE approx_time > {comp_time} ORDER BY block LIMIT 1) ORDER BY old::bigint DESC, block LIMIT 1;")
        if db.rowcount > 0:
            birds_record = db.fetchone()
            if records[period]['singular'][0] != birds_record[1] or records[period]['singular'][1] != birds_record[2]:
                db.execute(
                    f"UPDATE tg_record SET nft_id = '{birds_record[1]}', block = {birds_record[2]} WHERE record_period = '{period}' AND record_type = 'singular';")
                to_send_records['singular'][period] = birds_record
    conn.commit()

    for record_type in to_send_records:
        if 'monthly' in to_send_records[record_type]:
            send_record_message_to_channel(
                conn,
                bot,
                db,
                record_type,
                'monthly',
                to_send_records[record_type]['monthly'])
            ...
            send_record_message_to_channel(
                ...
                record_type,
                'weekly',
                to_send_records[record_type]['weekly'])
            ...
            send_record_message_to_channel(
                ...
                record_type,
                'daily',
                to_send_records[record_type]['daily'])
...
WHERE  t1.block > {tg_lastblock_v2} AND t1.nft_id=t3.nft_id AND t3.version = 'v2' AND t1.nft_id=t2.id AND t3.user_id=t4.id AND t4.is_active ORDER BY block);'''
    ...
    # Collect Sold NFTs
    db.execute(
        f"SELECT t1.nft_id, t1.old, t1.block, t2.metadata FROM nft_changes_vlite t1, nfts_vlite t2 WHERE  t1.block > {tg_lastblock_v1} AND t1.optype='BUY' AND t1.field='forsale' AND t1.nft_id=t2.id ORDER BY t1.block;")
    sold_list_singular = list(
        map(lambda x: [x[0], to_ksm(x[1], 'singular'), x[2], x[3]], db.fetchall()))
    db.execute(
        f"SELECT t1.nft_id, t1.old, t1.block, t2.metadata FROM nft_changes_v2 t1, nfts_v2 t2 WHERE  t1.block > {tg_lastblock_v2} AND t1.optype='BUY' AND t1.field='forsale' AND t1.nft_id=t2.id AND t2.collection IN ('{kanaria_birds_ids_str}') ORDER BY t1.block;")
    sold_list_birds = list(
        map(lambda x: [x[0], to_ksm(x[1]), x[2], x[3]], db.fetchall()))
    db.execute(
        f"SELECT t1.nft_id, t1.old, t1.block, t2.metadata FROM nft_changes_v2 t1, nfts_v2 t2 WHERE  t1.block > {tg_lastblock_v2} AND t1.optype='BUY' AND t1.field='forsale' AND t1.nft_id=t2.id AND t2.collection IN ('{kanaria_items_ids_str}') ORDER BY t1.block;")
    sold_list_items = list(
        map(lambda x: [x[0], to_ksm(x[1]), x[2], x[3]], db.fetchall()))
    ...
    for user_sold_params in users_sold_params:
        users_sold_params_dict[user_sold_params[0]] = {
            'singular_buy': user_sold_params[1], 'birds_buy': user_sold_params[2], 'items_buy': user_sold_params[3]}
    ...
    # Collect Listed NFTs
    db.execute(
        f"SELECT t1.nft_id, t1.new, t1.block, t2.metadata FROM nft_changes_v2 t1, nfts_v2 t2 WHERE  t1.block > {tg_lastblock_v2} AND t1.optype='LIST' AND t1.field='forsale' AND t1.nft_id=t2.id AND t1.new::bigint > 0.0 AND t2.collection IN ('{kanaria_birds_ids_str}') ORDER BY t1.block;")
    forsale_list_birds = list(
        map(lambda x: [x[0], to_ksm(x[1]), x[2], x[3]], db.fetchall()))
    db.execute(
        f"SELECT t1.nft_id, t1.new, t1.block, t2.metadata FROM nft_changes_v2 t1, nfts_v2 t2 WHERE  t1.block > {tg_lastblock_v2} AND t1.optype='LIST' AND t1.field='forsale' AND t1.nft_id=t2.id AND t1.new::bigint > 0.0 AND t2.collection IN ('{kanaria_items_ids_str}') ORDER BY t1.block;")
    forsale_list_items = list(
        map(lambda x: [x[0], to_ksm(x[1]), x[2], x[3]], db.fetchall()))
    ...
    for user_forsale_params in users_forsale_params:
        ...
        birds_forsale_by_user[user_forsale_params[0]]['any'], birds_forsale_by_user[user_forsale_params[0]]['limited'], birds_forsale_by_user[user_forsale_params[0]]['rare'], birds_forsale_by_user[user_forsale_params[0]]['super'], birds_forsale_by_user[user_forsale_params[0]]['founder'], items_forsale_by_user[user_forsale_params[0]
                                                                                                                                                                                                                                                                                                                       ]['any'], items_forsale_by_user[user_forsale_params[0]]['legendary'], items_forsale_by_user[user_forsale_params[0]]['epic'], items_forsale_by_user[user_forsale_params[0]]['rare'], items_forsale_by_user[user_forsale_params[0]]['uncommon'], items_forsale_by_user[user_forsale_params[0]]['common'] = [Decimal(x) for x in user_forsale_params[1:]]
    for forsale in forsale_list_birds:
        for user_id in birds_forsale_by_user:
            ...
            if birds_forsale['any'] >= forsale[1]:
                forsale_messages_list.append(
                    [user_id, 'bird', 'any', forsale[0], forsale[1], forsale[2], forsale[3]])
            if birds_forsale['super'] >= forsale[1] and 'KANS' in forsale[0]:
                forsale_messages_list.append(
                    [user_id, 'bird', 'super', forsale[0], forsale[1], forsale[2], forsale[3]])
            if birds_forsale['founder'] >= forsale[1] and 'KANF' in forsale[0]:
                forsale_messages_list.append(
                    [user_id, 'bird', 'founder', forsale[0], forsale[1], forsale[2], forsale[3]])
            if birds_forsale['rare'] >= forsale[1] and 'KANR' in forsale[0]:
                forsale_messages_list.append(
                    [user_id, 'bird', 'rare', forsale[0], forsale[1], forsale[2], forsale[3]])
    ...
    for forsale in forsale_list_items:
        ...
        for user_id in items_forsale_by_user:
            ...
            if items_forsale['any'] >= forsale[1]:
                forsale_messages_list.append(
                    [user_id, 'item', 'any', forsale[0], forsale[1], forsale[2], forsale[3]])
            if items_forsale[forsale_rarity] >= forsale[1]:
                forsale_messages_list.append(
                    [user_id, 'item', forsale_rarity, forsale[0], forsale[1], forsale[2], forsale[3]])
